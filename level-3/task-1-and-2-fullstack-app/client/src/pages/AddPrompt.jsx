    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <Link
            to="/prompts"
            className="p-2 rounded-xl bg-surface border border-surface-muted text-gray-400 hover:text-slate-500 transition-colors"
          >
      ...
      >
        <div className="bg-surface border border-surface-muted rounded-2xl p-6 shadow-glass space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              ...
              <input
                ...
                className="w-full bg-canvas border border-surface-muted rounded-xl px-4 py-2.5 text-slate-900 dark:text-white text-xs placeholder-gray-500 focus:ring-1 focus:ring-brand focus:border-brand"
                required
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={handleChange}
                className="w-full bg-canvas border border-surface-muted rounded-xl px-4 py-2.5 text-slate-900 dark:text-white text-xs focus:ring-1 focus:ring-brand focus:border-brand"
              >
                {categories.map((cat) => (
                  <option
                    key={cat}
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full bg-canvas border border-surface-muted rounded-xl p-4 text-slate-900 dark:text-white font-mono text-xs placeholder-gray-500 focus:ring-1 focus:ring-brand focus:border-brand leading-relaxed"
              required
            />
          </div>
              ...
              <div className="flex-1 overflow-hidden">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {detectedPlaceholders.map((v) => (
                    <span
                      key={v}
                      className="text-[10px] font-mono px-2 py-0.5 rounded bg-surface border border-brand/30 text-slate-900 dark:text-white"
                    >
                      {`{{${v}}}`}
                    </span>
                  ))}
          ...
          <div>
            ...
            <div className="relative flex items-center">
              ...
              <input
                ...
                className="w-full bg-canvas border border-surface-muted rounded-xl py-2.5 pl-10 pr-4 text-slate-900 dark:text-white text-xs placeholder-gray-500 focus:ring-1 focus:ring-brand focus:border-brand"
              />
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end space-x-3">
          <button
            ...
            className="px-4 py-2.5 rounded-xl text-xs font-medium text-gray-500 hover:text-slate-700 transition-colors"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={submitting}
            className="px-6 py-2.5 rounded-xl bg-brand hover:bg-brand-hover disabled:opacity-50 text-slate-900 dark:text-white text-xs font-semibold flex items-center space-x-2 shadow-glow transition-all"
          >
