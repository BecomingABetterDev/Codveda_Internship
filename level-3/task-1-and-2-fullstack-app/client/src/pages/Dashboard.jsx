...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const res = await API.get("/prompts", { params: { limit: 5 } });
        const list = res.data.data || [];
        setPrompts(list);

        // Aggregate vault stats safely
        const total = res.data.pagination?.total || list.length;
        const favorites = list.filter((p) => p.isFavorite || p.favorite).length;

        // Extract normalized category names for unique category count
  ...
  return (
    <div className="space-y-8">
      {/* Banner */}
      <div className="relative rounded-2xl bg-surface border border-surface-muted p-6 sm:p-8 overflow-hidden shadow-glass">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Link
            to="/prompts/new"
            className="bg-brand hover:bg-brand-hover text-white text-xs font-semibold px-4 py-3 rounded-xl flex items-center justify-center space-x-2 shadow-glow transition-all self-start sm:self-auto"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {statsCards.map((stat, idx) => {
          ...
          return (
            <div
              key={idx}
              className="bg-surface border border-surface-muted p-5 rounded-2xl shadow-glass"
            >
      ...
      {/* Recent Vault Shelf */}
      <div className="bg-surface border border-surface-muted rounded-2xl p-6 shadow-glass">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {prompts.map((prompt) => {
              ...
              // Normalize Category Label String
              const categoryLabel =
              ...
              return (
                <div
                  ...
                  className="p-4 rounded-xl bg-canvas border border-surface-muted flex items-center justify-between hover:border-brand/50 transition-colors cursor-pointer group"
                >
                  <div className="flex-1 pr-4 min-w-0">
                    <div className="flex items-center space-x-2.5 mb-1">
                      {/* Fixed Legible Category Badge */}
                      ...
                      <h3 className="text-xs font-bold text-white group-hover:text-brand transition-colors truncate">
                        {prompt.title}
                      </h3>
                    </div>
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg border text-xs font-semibold flex items-center space-x-1.5 transition-all shrink-0 ${
                      isCopied
                        ...
                        : "bg-surface border-surface-muted text-gray-300 hover:bg-brand hover:text-white hover:border-brand"
                    }`}
                  >
                    {isCopied ? (
                      <>
